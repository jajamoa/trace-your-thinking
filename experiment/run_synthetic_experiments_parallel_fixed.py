#!/usr/bin/env python3
"""
Fixed parallel run experiments with synthetic agents
"""
import sys
import os
import json
...
class SyntheticExperimentRunner:
    ...
    def __init__(self, agent_data_dir="agent_data/synthetic_agents", 
        ...
        self.max_workers = max_workers or min(multiprocessing.cpu_count(), 4)  # Cap at 4 workers
    ...
    def run_all_experiments(self):
        ...
        try:
            with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                # Submit all tasks
                print(f"Submitting {len(tasks)} tasks to {self.max_workers} workers...")
                future_to_task = {}
                ...
                try:
                    for future in as_completed(future_to_task):
                        ...
                        try:
                            ...
                            failed += 1
                            print(f"TIMEOUT: {agent_id} - {topic} (exceeded 2 minutes) [{completed}/{total_tasks} {progress_pct:.1f}%]")
                            results.append({
                                "agent_id": agent_id,
                                "topic": topic,
                                "status": "error",
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Run synthetic agent experiments (parallel)')
