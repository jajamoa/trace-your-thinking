...
echo -e "${CYAN}Starting Synthetic Agent Experiments${NC}"
echo -e "${CYAN}====================================${NC}"
...
echo -e "\n${PURPLE}Running experiments...${NC}"
echo -e "${PURPLE}======================${NC}"

# Run experiments with better output formatting (using ultra-parallel version)
python run_experiments_ultra_parallel.py \
    --agent-dir agent_data/synthetic_agents \
    --topics zoning healthcare surveillance \
    --max-qa 20 \
    --workers 12 \
    --llm-threads 16 \
