...
class LLMSyntheticAgent(BaseAgent):
    """Agent that uses LLM with CBN-based personality"""
    
    def __init__(self, agent_id, agent_data_path, use_llm=True):
        """
        Initialize LLM synthetic agent
    ...
    def set_topic(self, topic):
        ...
        # Load CBN prompt for this topic (this is all we need for reasoning)
...
Provide a brief, authentic response (under 100 words) based on your beliefs."""
            ...
            data = {
                ...
                "parameters": {
                    "temperature": 0.3,  # Lower temperature for more consistent survey responses
                    "max_tokens": 50 if question_type in ["stance", "opinion", "multiple_choice"] else 150,
                    "top_p": 0.8
                }
            }
...
# Monkey patch to use LLM agent by default
def create_synthetic_agent(agent_id, agent_data_path, use_llm=True):
    ...
    if use_llm and os.getenv('DASHSCOPE_API_KEY'):
        return LLMSyntheticAgent(agent_id, agent_data_path, use_llm=True)
    else:
